...
def get_training_imgs_info(
    ...
    return images_info, action_images_cnt
...
class ReadValidImagesAndActionTypesByTxt(object):
    ...
    def __init__(self, img_folder, valid_imgs_txt,
        ...
        """
        self.images_info, self.labels_info = get_training_imgs_info(
...
def load_skeleton_data(filepath, classes):
    ...
    with open(filepath, 'r') as f:
        ...
        for i, row in enumerate(dataset):
            sliced_row = row[LEN_IMG_INFO:LEN_IMG_INFO + LEN_SKELETON_XY]
            if len(sliced_row) != 34:
                print(f"Warning: Skipping row {i} with length {len(sliced_row)}. Expected length: 34.")
                print(row)
            else:
                X.append(sliced_row)
        X = np.array(X)
